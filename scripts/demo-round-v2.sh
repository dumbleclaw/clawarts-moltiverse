#!/bin/bash
# demo-round-v2.sh — Simulates a full Clawarts round with $STEALTH ERC-20 token
# Usage: ./demo-round-v2.sh [ROUND_ID] [THEME]
# Example: ./demo-round-v2.sh ROUND-011 "AI Tutor for Kids"
set -euo pipefail

export PATH=$HOME/.foundry/bin:$PATH
STEALTH=0x0B306BF915C4d645ff596e518fAf3F9669b97016
...
# Believers (address, name, idea_index, amount)
BELIEVERS=(
...
# Challengers (address, name, idea_index, amount)
CHALLENGERS=(
...
print_pool() {
  echo -e "${C_DIM}   📊 Pool: ${POOL_TOTAL} \$STEALTH | Believers: ${BELIEVE_TOTAL} (${BELIEVE_COUNT}) | Challengers: ${CHALLENGE_TOTAL} (${CHALLENGE_COUNT})${C_RESET}"
}
...
echo -e "${C_MAGENTA}║  🔮 CLAWARTS — DEMO ROUND V2 ($STEALTH)     ║${C_RESET}"
...
# ═══════════════════════════════════════════════════════════════
# PHASE 3: Stakes via Impersonation (ERC-20)
# ═══════════════════════════════════════════════════════════════
echo ""
echo -e "${C_MAGENTA}━━━ FASE 3: Believes & Challenges with \$STEALTH (13 stakes) ━━━${C_RESET}"

setup_account() {
  local addr="$1" amount_eth="$2"
  # Fund with ETH for gas
  cast rpc anvil_setBalance "$addr" "0x56bc75e2d63100000" --rpc-url $RPC > /dev/null 2>&1
  # Impersonate deployer to mint STEALTH tokens
  cast send $STEALTH "mint(address,uint256)" "$addr" $(cast --to-wei 100) \
    --private-key $ORCHESTRATOR_KEY --rpc-url $RPC --quiet 2>&1 | tail -1
  # Impersonate account and approve
  cast rpc anvil_impersonateAccount "$addr" --rpc-url $RPC > /dev/null 2>&1
  cast send $STEALTH "approve(address,uint256)" $CONTRACT $(cast --to-wei 100) \
    --from "$addr" --unlocked --rpc-url $RPC --quiet 2>&1 | tail -1
}
...
do_stake() {
  ...
  local amount_wei=$(cast --to-wei "$amount")

  # Setup: fund gas, mint STEALTH, approve
  setup_account "$addr" "$amount"

  if [ "$is_challenge" = "true" ]; then
    echo -e "${C_RED}⚔️  ${name} desafía \"${title}\" con ${amount} \$STEALTH${C_RESET}"
    cast send $CONTRACT "challenge(bytes32,bytes32,uint256)" $ROUND_BYTES $idea_bytes $amount_wei \
      --from "$addr" --unlocked --rpc-url $RPC --quiet 2>&1 | tail -1
    ...
    echo -e "${C_GREEN}🙌 ${name} cree en \"${title}\" con ${amount} \$STEALTH${C_RESET}"
    cast send $CONTRACT "believe(bytes32,bytes32,uint256)" $ROUND_BYTES $idea_bytes $amount_wei \
      --from "$addr" --unlocked --rpc-url $RPC --quiet 2>&1 | tail -1
    add_believe "$amount"
...
echo -e "${C_GREEN}✅ ${TOTAL_STAKES} stakes registrados onchain con \$STEALTH${C_RESET}"
sleep 1
...
echo -e "${C_GREEN}🏆 ¡\"${winner_title}\" gana la ronda! Treasury: +${TREASURY} \$STEALTH${C_RESET}"
sleep 1
...
echo -e "${C_MAGENTA}║  Pool:         ${POOL_TOTAL} \$STEALTH${C_RESET}"
echo -e "${C_MAGENTA}║  Believers:    ${BELIEVE_COUNT} (${BELIEVE_TOTAL} \$STEALTH)${C_RESET}"
echo -e "${C_MAGENTA}║  Challengers:  ${CHALLENGE_COUNT} (${CHALLENGE_TOTAL} \$STEALTH)${C_RESET}"
echo -e "${C_MAGENTA}║  Spells:       ${SPELL_COUNT}${C_RESET}"
echo -e "${C_MAGENTA}║  Treasury:     ${TREASURY} \$STEALTH${C_RESET}"
echo -e "${C_MAGENTA}║  Participantes: ${PARTICIPANTS}${C_RESET}"
echo -e "${C_MAGENTA}║  Token:        \$STEALTH (${STEALTH})${C_RESET}"
echo -e "${C_MAGENTA}║  Engine:       WorldEngineV2 (${CONTRACT})${C_RESET}"
echo -e "${C_MAGENTA}╚══════════════════════════════════════════════╝${C_RESET}"
echo ""
